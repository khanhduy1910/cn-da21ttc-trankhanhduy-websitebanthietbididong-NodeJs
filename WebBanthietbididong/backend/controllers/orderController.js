...
exports.updateOrder = catchAsyncErrors(async (req, res, next) => {
    ...
    if (order.orderStatus === 'Đã giao hàng') {
        return next(new ErrorHandler('Đơn hàng này đã được xử lý', 400));
    }

    // Cập nhật trạng thái và số lượng sản phẩm đã bán
...
// Hàm cập nhật số lượng trong kho và số lượng đã bán
async function updateStock(id, quantity) {
    const product = await Product.findById(id);
    product.stock -= quantity;  // Giảm số lượng trong kho
    product.sold += quantity;  // Tăng số lượng đã bán
